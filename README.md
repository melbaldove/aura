...
- **External hooks** — standalone processes stream alerts, events, and button-asks into Aura over the local ctl socket (`aura hook run` + rules files), so scripts can surface attention and request decisions without Discord access.
