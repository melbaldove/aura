...
3. **Every tool call produces a visible outcome.** Success or error — never silence. The outcome must be durable and inspectable by the brain and the user. Visibility does not require an immediate user-facing message. Attention policy decides if Aura records, digests, or surfaces the outcome.
4. **Flare work state and executor state are separate.** A flare and each execution attempt are each in exactly one valid state. Every transition writes a durable audit event. The brain completes a flare only after it validates the completion requirements and proof. User acceptance is required only when the objective defines it as a completion requirement. An ACP `end_turn` event is a turn boundary, not executor-process completion. Routine transitions do not require a Discord message. Attention policy decides what Aura surfaces.
...
6. **Active execution attempt ↔ active monitor.** A running execution attempt has exactly one runtime and one monitor actor. The monitor lives as long as the attempt runs. It stops when the attempt stops. A waiting, paused, interrupted, completed, failed, or cancelled attempt has no active runtime or monitor. Resume creates or reconnects one valid attempt according to the executor contract. No orphaned monitors and no zombie runtimes are permitted.
