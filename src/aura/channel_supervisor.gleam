      ...
      case dict.get(state.children, channel_id) {
        Ok(existing) -> {
          case process.is_alive(existing.pid) {
            ...
            False -> {
              let cleaned = remove_child(state, channel_id)
              start_child(cleaned, channel_id, deps, reply)
            }
          }
        }
        Error(Nil) -> start_child(state, channel_id, deps, reply)
      }
    }

    ChildDown(channel_id:) -> {
      actor.continue(remove_child(state, channel_id))
    }
      ...
      let next_state = case down {
        process.ProcessDown(monitor:, ..) ->
          case find_channel_for_monitor(state.children, monitor) {
            Ok(channel_id) -> remove_child(state, channel_id)
            Error(Nil) -> state
          }
    ...
    Ok(child_subject) -> {
      case process.subject_owner(child_subject) {
        Ok(pid) -> {
          ...
          let children = dict.insert(state.children, channel_id, child)
...
fn remove_child(state: SupervisorState, channel_id: String) -> SupervisorState {
  case dict.get(state.children, channel_id) {
    Ok(child) -> {
...
fn find_channel_for_monitor(
  ...
  |> list.find_map(fn(entry) {
    case entry {
      #(channel_id, child) if child.monitor == monitor -> Ok(channel_id)
      _ -> Error(Nil)
    }
  })
}
