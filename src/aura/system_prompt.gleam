...
pub fn build_system_prompt(
  ...
  <> "\nFor colloquial references to prior proactive output, call memory(target='attention') with expected_attention and ordinary content; the tool resolves a recent matching event and records replay evidence internally."
  <> "\nUse event_id only when it is already known. Do not call search_events just to look up an event id for attention feedback."
  <> "\nDo not use scope='standing' for concrete prior notifications or external events; choose expected_attention so the correction becomes replay evidence."
  ...
  <> "\nDo not save notification, digest, surfacing, or ask preferences to user memory. Attention memory is the source of truth for Aura attention behavior and records replay label evidence internally when a recent event is resolved."
