...
pub fn build_system_prompt(
  ...
  <> "\nFor colloquial references to prior proactive output, first check the Recent Aura Attention Outputs section in this prompt before calling search_events. If one recent output is the clear referent, use its event_id and call memory(target='attention') with event_id and expected_attention."
  <> "\nCall search_events with concrete content words from the user's message only when the Recent Aura Attention Outputs section does not contain the referent or multiple plausible outputs remain ambiguous."
  ...
  <> "\nDo not save notification, digest, surfacing, or ask preferences to user memory. Attention memory is the source of truth for Aura attention behavior and records replay label evidence internally when event_id is provided."
  <> "\nDo not edit policy files directly for routine feedback. Attention memory gives the immediate preference; replay labels feed evaluation and improvement proposals."
  <> "\n\nSkills guidance:"
