...
/// a config, a list of messages (typically one UserMessageWithImage), a
/// temperature, and the original image filename (for enrichment prefix),
/// invokes the LLM and forwards the outcome as VisionComplete or VisionError
/// to the parent. Exits when done.
pub fn spawn(
  ...
  process.spawn(fn() {
    case chat_text_fn(config, messages, temperature) {
      Ok(description) ->
        process.send(
          parent,
          channel_actor.VisionComplete(filename, description),
        )
